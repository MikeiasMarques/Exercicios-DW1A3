<!DOCTYPE html>

  <html lang="pt-br">

<head>
    <meta charset="utf-8"/>
   <title>Desenvolvimento Web</title>
</head>

<!DOCTYPE html>

  <html lang="pt-br">

<head>
    <meta charset="utf-8"/>
   <title>Desenvolvimento Web</title>
</head>

<body>
<color>
   <h1> Exercicio 1 - textos em HTML </h1>

<h3> Site criado para explicação de tags </h3>

  <br>
  
  <p> <strong> strong </p> 
    <p>Tarefa de desenvolvimento web para ser entregue <strong>  até às 22:30  </p>
<!DOCTYPE html>

  <html lang="pt-br">

<head>
    <meta charset="utf-8"/>
   <title>Desenvolvimento Web</title>
</head>

<body bgcolor="mikeias">
<color>
   <h1> Exercicio 1 - textos em HTML </h1>

<h3> Site criado para explicação de tags </h3>

  <br>
  
  <p> <strong> strong </p> 
    <p>Tarefa de desenvolvimento web para ser entregue <strong>  até às 22:30  </p>
  <hr>
  
  <p> <strong> em </p> 
  <p>Hoje você <em> não</em> vai faltar! </p>
  
  <hr>
  
  <p> <strong> blockquote </p> 
  
  <blockquote>
    "Tornou-se aterradoramente claro que a nossa tecnologia ultrapassou a nossa Humanidade"
 (Albert Einstein - Fisico)  
  </blockquote>
  
  <hr>
  <p> <strong> q </p>
  
  <p>  <q> E que a sorte esteja sempre a seu favor </q> </p>
  <hr>
  
  <p> <strong> abbr </p>
  <p>Donald trump  é presidente dos <abbr title="Estados Unidos da América">EUA</abbr></p>
  <hr>
  
<p> <strong> cite </p>
<cite> (Effie Trinket em Jogos Vorazes) </cite>
  <hr>
  
  <p> <strong> dfn </p>
  <p><dfn >HTML</dfn> é uma linguagem para criar paginas web </p>
     <hr>
  
   <p> <strong> address </p>
  <p> email: <address> mikeias.m@aluno.ifsp.edu.br</address> </p>
  <p> Tel: <address> 59340126</address> </p>
    <hr>
  
  
    <p> <strong> ins/del/s </p>
  <p>Eu <s>sempre </s>prefiro  o <del>frio</del> <ins>calor</ins></p>
    <hr>
   
    
     <p> <strong>sup </p>
       <p>2<sup>a</sup>feira não haverá aula.</p>
    <hr>
    
     <p> <strong>sub </p>
    <p>A composição química da água é H<sub>2</sub>O.</p>
    
  
</body>

</html>
